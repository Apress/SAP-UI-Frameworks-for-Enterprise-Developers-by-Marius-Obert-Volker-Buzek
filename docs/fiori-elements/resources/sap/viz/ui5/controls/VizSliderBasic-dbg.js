...
sap.ui.define([
	...
	function(
		...
		var Slider = Control.extend("sap.viz.ui5.controls.VizSliderBasic", { metadata: {
		...
		Slider.prototype._recalculateStyles = function() {
			var $Slider = this.$(),
				oHandle = this.$().find("." + this.getRenderer().CSS_CLASS + "Handle").eq(0),
		...
		Slider.prototype._handleTickmarksResponsiveness = function () {
			var aLabelsInDOM, fOffsetLeftPct, fOffsetLeftPx, aHiddenLabels,
				...
				aTickmarksInDOM = this.$().find("." + this.getRenderer().CSS_CLASS + "Tick"),
				iScaleWidth = this.$().find("." + this.getRenderer().CSS_CLASS + "Tickmarks").width(),
				bShowTickmarks = (iScaleWidth / aTickmarksInDOM.length) >= this._CONSTANTS.TICKMARKS.MIN_SIZE.SMALL;
			...
			aLabelsInDOM = this.$().find("." + this.getRenderer().CSS_CLASS + "TickLabel").toArray();
		 ...
		 *
		 * @returns {Element} The nearest handle DOM reference.
		 */
		 ...
		 * @param {int} fIncrement default 1
		 * @private
		 */
		 ...
		 * @param {int} fDecrement default 1
		 * @private
		 */
		 ...
		 * @param {sap.ui.core.Control} oAriaLabel Control that will be used as reference for the screen reader
		 * @returns {sap.m.Input} Input control with all needed events attached and properties filled
		 * @private
		 */
		...
		Slider.prototype.onfocusout = function(oEvent) {
			...
			this.$("progress").removeClass("focus");
		};
