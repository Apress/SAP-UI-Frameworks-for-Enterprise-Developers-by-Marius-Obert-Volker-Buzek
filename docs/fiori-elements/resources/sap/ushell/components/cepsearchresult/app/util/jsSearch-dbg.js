...
sap.ui.define([], function () {
  ...
  return function JsSearch(options) {
    ...
    if (
      ...
      !Array.isArray(options.algorithm.options) ||
      typeof options.algorithm.options[0] !== "number" ||
      typeof options.algorithm.options[1] !== "number" ||
      typeof options.algorithm.options[2] !== "number" ||
      typeof options.algorithm.options[3] !== "number" ||
      typeof options.algorithm.options[4] !== "number" ||
      typeof options.algorithm.options[5] !== "number" ||
      typeof options.algorithm.options[6] !== "number"
    ) {
    ...
    if (Array.isArray(options.fields)) {
      ...
      if (Array.isArray(options.fields.groups)) {
        ...
        options.fields.groups.forEach(function (g, i) {
          var weight =
            ((options.fields.groups.length - 1 - i) / (options.fields.groups.length - 1)) *
            (scale - 1) +
            1;
          if (Array.isArray(g)) {
    ...
    highlight.regExpfillOneCharacterGaps = makeRegExp(
      highlight.highlightChar + "(?!" + highlight.highlightChar + ")[^a0]" + highlight.highlightChar,
      "g"
    );
    /** @param {string} s */
    ...
    highlight.applyBitmap = function (s, bitmap) {
      ...
      bitmap = bitmap.replace(
        highlight.regExpfillOneCharacterGaps,
        highlight.highlightChar + highlight.highlightChar + highlight.highlightChar
      );
      // insert <b>'s
    ...
    function _algorithms(a, opt) {
      var toBeReturned = {
        "contains-ranked": {
          ...
          compileSearchTerm: function (searchTerm) {
            // data structure to work on
            /** @type {searchTermHalfCompiled} */
            var st = {
              original: options.shouldNormalize ? searchTerm.normalize("NFC") : searchTerm, // original string (unicode normalized)
              parsed: [],
            ...
            function lookBehindEndOfWord(lastSearchWord) {
              ...
              return "(.*)"; // This might not be 100% correct, but is the best we can do without lookbehinds
                        ...
                        /** @param {mapTypeFunction} f */ function mapType(a, type, f) {
            ...
            var a = st.original.replace(/\\\"/g, "\u0010").split('"'); // Escape escaped quotes and split at quotes
            ...
            mapType(
              ...
              "word",
                ...
                for (var i = 0; i < w.children.length; i++) {
                  if (
                    i > 0 &&
                    w.children[i].cOriginal.match(regExps.AlphaNum_atBegin) &&
                    w.children[i - 1].cOriginal.match(regExps.AlphaNum_atBegin)
                  ) {
                ...
                s.children = s.children.filter(
                  ...
                  function (y, i) {
                    return (
                      ...
                      !s.children[i].wOriginal.match(/^\*+$/) ||
                      !s.children[i + 1].wOriginal.match(/^\*+$/)
                    );
                  }
                );
                s.children = s.children.filter(
                ...
                if (!s.children.length) {
                    ...
                    function (word, i, pre, succ) {
                      if (word.wOriginal !== "*") {
                        ...
                        if (
                          ...
                          if (
                            !(
                              pre.wOriginal.match(regExps.Alpha_atEnd) &&
                              word.wOriginal.match(regExps.Alpha_atBegin)
                            ) &&
                            !(
                              pre.wOriginal.match(regExps.Num_atEnd) &&
                              word.wOriginal.match(regExps.Num_atEnd)
                            )
                          ) {
                        ...
                        lastCharacterOriginal(pre).match(regExps.Alpha_atBegin) &&
                        firstCharacterOriginal(succ).match(regExps.Alpha_atBegin)
                      ) {
                        //  letter * letter
                        middle += "(?:[^a]|[^a].*?[^a])";
                      } else if (
                        lastCharacterOriginal(pre).match(regExps.Num_atEnd) &&
                        firstCharacterOriginal(succ).match(regExps.Num_atEnd)
                      ) {
                        //  number * number
                        middle += "(?:[^0]|[^0].*?[^0])";
                      } else if (
                  ...
                  ret.push({
                    r:
                      ...
                      middle +
                      ")" +
                      lookBehindEndOfWord(z.wOriginal) +
                      "$",
                    score: opt[0],
                ...
                if (s.wOriginal.match("^\\*+$")) {
                  ...
                  ret.push({
                    r:
                      ...
                      s.wRegExp +
                      ")" +
                      lookBehindEndOfWord(s.wOriginal) +
                      "$",
                    score: opt[0],
                    applyHighlighting: true
                  });
          ...
          compileTestTerm: function (testTerm) {
          ...
          for (k = 0; k < searchTermWord.r.length; k++) {
            ...
            if (testTermsCompiled[j].normalized.match(searchTermWordRegExp.r)) {
              ...
              if (searchTermWordRegExp.applyHighlighting) {
                ...
                bitmaps[j] = highlight.calcBitmap(
                  testTermsCompiled[j].normalized,
                  bitmaps[j],
                  searchTermWordRegExp.r
                );
              }
              scorePerTestTerm = Math.max(
                scorePerTestTerm,
                searchTermWordRegExp.score * testTermsCompiled[j].weight
              );
            }
          }
      ...
      for (j in highlighted) {
        ...
        if (testTermsCompiled[j].useNormalizationMap) {
          for (k = 0; k < testTermsCompiled[j].normalizationMap.length; k++) {
            bitmaps[j] =
              bitmaps[j].substr(0, k + 1) +
              bitmaps[j].substr(k + testTermsCompiled[j].normalizationMap[k]);
          }
        }
        // apply bitmap
