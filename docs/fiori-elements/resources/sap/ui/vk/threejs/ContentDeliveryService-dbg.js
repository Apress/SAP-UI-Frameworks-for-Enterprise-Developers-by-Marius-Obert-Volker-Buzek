...
sap.ui.define([
	...
	ContentDeliveryService.prototype.initUrl = function(url, keepCachedData, correlationId) {
		...
		if (!this._loader.running() || this._loader.getUrl() !== url) {
			...
			this._loader.setCorrelationId(correlationId ? correlationId : THREE.MathUtils.generateUUID().toLowerCase());
	...
	ContentDeliveryService.prototype.assignMaterialToNodes = function(sceneId, material, nodeRefs, assignToRestOfSceneTree) {
		var that = this;
		return this._loader.requestMaterial(sceneId, material).then(function(materialRef) {
	...
	ContentDeliveryService.prototype.replaceMaterialOnNodes = function(sceneId, originalMaterial, replacementMaterial, nodeRefs) {
		...
		return Promise.all([this._loader.requestMaterial(sceneId, originalMaterial), this._loader.requestMaterial(sceneId, replacementMaterial)]).then(function(values) {
			var originalMaterialRef = values[0];
			var replacementMaterialRef = values[1];

			function replaceMaterial(nodeRef, originalMaterialRef, replacementMaterialRef) {
				if (!nodeRef) {
					return;
				}
				...
				if (that._currentNodeHierarchy) {
					var nodeProxy = that._currentNodeHierarchy.createNodeProxy(nodeRef);
					nodeProxy.replaceMaterial(originalMaterialRef, replacementMaterialRef);
					that._currentNodeHierarchy.destroyNodeProxy(nodeProxy);
				}

				nodeRef.children.forEach(function(childRef) {
					replaceMaterial(childRef, originalMaterialRef, replacementMaterialRef);
				});
			}

			if (!nodeRefs) {
				// replace on whole scene
				var context = that._loader.getContext(sceneId);
				replaceMaterial(context.root, originalMaterialRef, replacementMaterialRef);
			} else if (Array.isArray(nodeRefs)) {
				nodeRefs.forEach(function(nodeRef) {
					replaceMaterial(nodeRef, originalMaterialRef, replacementMaterialRef);
				});
			} else {
				replaceMaterial([nodeRefs], originalMaterialRef, replacementMaterialRef);
			}
