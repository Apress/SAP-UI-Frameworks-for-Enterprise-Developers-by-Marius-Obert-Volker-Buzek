	...
	function build() {
		...
		svg.Element.a = function(node) {
			...
			this.onclick = function() {
				...
				// originally the function has just one line:
				// window.open(this.attribute('xlink:href').value);
				// we change it because of security reasons
				var sWindowFeatures = "noopener,noreferrer";
				var uaNav = window.navigator.userAgent;
				if (/MSIE|Trident|Edge/.test(uaNav)) { // it is not true for chromium edge
					var oNewWindow = window.open("about:blank", undefined, sWindowFeatures);
					if (oNewWindow) {
						oNewWindow.opener = null;
						oNewWindow.location.href = this.attribute('xlink:href').value;
					}
				} else {
					window.open(this.attribute('xlink:href').value, undefined, sWindowFeatures);
				}
				// ##### END: MODIFIED BY SAP
			}
